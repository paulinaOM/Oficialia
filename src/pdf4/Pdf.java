...
public class Pdf {
    public void createPdf(String dest, List<Pdf4> pdfList) throws IOException {//Recibe la carpeta de destino y la lista de empleados autilizar para la
        //...creación del PDF
        ...
        Paragraph p = new Paragraph("Documentos entregados por fecha");
        // Add Paragraph to document
        document.add(p);

        Table table = new Table(new float[]{4, 3, 3,4,4,4});//Indicalas columnas que va a generar, ancho de la columna
    ...
    public void process(Table table, Pdf4 registro, PdfFont font) {
        ...
        table.addCell(new Cell().add(new Paragraph(registro.getNombreRecibe()).setFont(font)));
        table.addCell(new Cell().add(new Paragraph(registro.getFechaEntrega().toString()).setFont(font)));
